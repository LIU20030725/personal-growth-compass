import { Hammer, House, Mail, Music2, Sparkles } from 'lucide-react';
...
export function HomeView({ investments, reducedMotion, hasWindValleyDiscovery, onInvest }: HomeViewProps) {
  ...
  return (
    <section className={`journal-view home-view ${reducedMotion ? 'is-reduced-motion' : ''}`} id="journal-home-panel" role="tabpanel" aria-label="永久之家" aria-labelledby="journal-home-tab">
      <div className="journal-world-shell home-world-shell">
        <div className="journal-world-heading">
          <div><span>永久之家</span><h2>风铃小院</h2></div>
          <div className="journal-weather"><House size={16} aria-hidden="true" />今日安稳 · {completed}/2 建成</div>
        </div>
      ...
      <aside className="home-project-panel" aria-label="家园建造项目">
        <header><span>家园建造</span><h2>让院子慢慢长大</h2></header>
        ...
        {hasWindValleyDiscovery ? (
          <article className="home-discovery-card">
            <span aria-hidden="true"><Music2 size={22} /></span>
            <div><small>旅途发现</small><h3>山谷风铃</h3><p>从风过山谷带回的第一件纪念物。微风经过院子时，它会轻轻响起。</p></div>
          </article>
        ) : null}
