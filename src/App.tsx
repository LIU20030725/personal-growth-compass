...
const accountGroups: AccountGroupMeta[] = [
  ...
  },
  {
    ...
    title: "理财账户",
    addLabel: "增加账户",
    description: "股票、基金、港股券商、美股券商等理财账户",
    icon: LineChart,
  },
  {
    id: "receivable",
...
function getMonthlyTrend(
  ...
  return days.map((label) => {
    ...
    const entries = transactions.filter(
      (item) => item.date <= date && item.date.startsWith(period),
    );
    return {
      label,
      income: sum(
...
function getExpenseCategories(
  transactions: Transaction[],
): Array<{ category: string; amount: number; share: number }> {
  const expenses = transactions.filter(
    (item) => item.type === "expense" && item.date.startsWith(period),
  );
...
function getMonthlySavingsCalendar(transactions: Transaction[], year: string) {
  return Array.from({ length: 12 }, (_, index) => {
    ...
    const monthTransactions = transactions.filter((item) =>
      item.date.startsWith(monthPeriod),
    );
    const income = sum(
      monthTransactions
        .filter((item) => item.type === "income")
...
function summarizeActivitiesByMonth(activities: AccountActivity[]) {
  const groups = activities.reduce<Record<string, AccountActivity[]>>(
    (result, activity) => {
  ...
  return Object.entries(groups)
    .map(([month, items]) => ({
      ...
      income: sum(
        items
          .filter((item) => isActivityInflow(item.type))
          .map((item) => item.amount),
      ),
      expense: sum(
        items
...
function groupActivitiesByDate(activities: AccountActivity[]) {
  return activities.reduce<Array<{ date: string; items: AccountActivity[] }>>(
    (groups, activity) => {
...
function getMonthTransactions(
  ...
  return {
    items,
    income: sum(
      items.filter((item) => item.type === "income").map((item) => item.amount),
    ),
    expense: sum(
      items
        .filter((item) => item.type === "expense")
  ...
  const toPolyline = (key: "income" | "expense") =>
    points
      .map((point, index) => {
        ...
        const y =
          height - padding - (point[key] / maxValue) * (height - padding * 2);
        return `${x},${y}`;
      })
      .join(" ");
...
function MonthlySavingsCalendar({
  ...
  return (
    <section
      className="panel savings-calendar"
      aria-label={`${currentYear} 年月度存钱日历`}
    >
      <div className="section-heading">
        <div>
          <p>存钱日历</p>
      ...
      <div className="calendar-summary">
        <div>
          <span>年度累计</span>
          <strong className={totalSavings >= 0 ? "positive" : "negative"}>
      ...
      <div className="month-grid">
        {months.map((item, index) => (
          ...
          >
            ...
            <small>
              收 {formatCurrency(item.income)} / 支{" "}
    ...
    <section className="character-status-view" aria-label="人物状态信息表">
      <div className="character-status-hero panel">
        ...
        <div className="character-status-copy">
          <p className="eyebrow">
            <Sparkles size={17} /> Character Status
          </p>
          <h1>人物状态</h1>
          ...
          <p>
            把现实里的每一次行动记录成经验值，让财富、能力、身体与情绪在同一张角色表里持续升级。
          </p>
          <div className="level-progress" aria-label="当前等级经验值 68%">
            <span>
              <b>LV.42</b>
              <small>6,820 / 10,000 XP</small>
            </span>
        ...
        <section className="panel status-sheet">
          ...
          <div className="quest-list">
            <article>
              <span>01</span>
              <div>
                <strong>建立稳定的晨间系统</strong>
                <small>连续完成 5 / 7 天</small>
              </div>
              <b>+120 XP</b>
            </article>
            <article>
              <span>02</span>
              <div>
                <strong>完成本月成长复盘</strong>
                <small>整理财富、能力与健康记录</small>
              </div>
              <b>+200 XP</b>
            </article>
            <article>
              <span>03</span>
              <div>
                <strong>推进年度核心项目</strong>
                <small>本周进度 3 / 5</small>
              </div>
              <b>+300 XP</b>
            </article>
          </div>
...
const moduleViewContent: Record<
  Exclude<MainView, "finance" | "character">,
  { eyebrow: string; title: string; description: string; items: string[] }
> = {
  ability: {
    eyebrow: "Ability Tree",
    title: "能力属性",
    description: "用技能树管理专业能力、通用能力与长期练习。",
    items: ["专业技能树", "本周练习", "能力里程碑"],
  },
  body: {
    eyebrow: "Body Status",
    title: "健康状况",
    description: "记录睡眠、运动与身体指标，保持稳定输出。",
    items: ["睡眠恢复", "运动计划", "身体数据"],
  },
  emotion: {
    eyebrow: "Mind Status",
    title: "情绪状态",
    description: "用轻量记录观察压力、能量与情绪波动。",
    items: ["今日情绪", "压力来源", "恢复行动"],
  },
  quests: {
    eyebrow: "Quest Board",
    title: "任务",
    description: "组织主线、支线与每日任务，让行动直接转化为经验值。",
    items: ["今日任务", "本周主线", "待领取奖励"],
  },
  achievements: {
    eyebrow: "Achievement Hall",
    title: "成就",
    description: "收藏真实进步形成的徽章、称号与人生里程碑。",
    items: ["最新解锁", "成就图鉴", "里程碑"],
  },
  journal: {
    eyebrow: "Adventure Journal",
    title: "冒险日志",
    description: "汇总每日行动、复盘与成长轨迹，留下可回看的个人史。",
    items: ["今日记录", "周度复盘", "成长时间线"],
  },
};
...
function ModuleView({
  ...
  view: Exclude<MainView, "finance" | "character">;
}) {
...
export default function App() {
  ...
  const [expandedGroup, setExpandedGroup] = useState<AccountGroupId | null>(
    "cash",
  );
  ...
  const [openAccountMonth, setOpenAccountMonth] = useState<string | null>(
    period,
  );
  ...
  const financeState = useMemo(
    () => buildFinanceState(accounts, transactions),
    [accounts, transactions],
  );
  const overview = useMemo(
    () => calculateFinanceOverview(financeState, period),
    [financeState],
  );
  ...
  const groupTotal = sum(
    accountGroups.map((group) =>
      sum(
        accounts
          .filter((account) => account.group === group.id)
  ...
  const selectedAccountActivities = selectedAccount
    ...
    : [];
  const selectedAccountMonths = summarizeActivitiesByMonth(
    selectedAccountActivities,
  );
  const operationLabels = getOperationLabels(selectedAccount);
  const transferTargets = selectedAccount
  ...
  const ledgerDates = useMemo(
    () =>
      Array.from(
        new Set(periodTransactions.map((transaction) => transaction.date)),
      ).sort((a, b) => b.localeCompare(a)),
    [periodTransactions],
  );
  const activeLedgerDate = ledgerDates.includes(selectedLedgerDate)
    ? selectedLedgerDate
    : (ledgerDates[0] ?? "");
  const activeDateTransactions = periodTransactions.filter(
    (transaction) => transaction.date === activeLedgerDate,
  );
  const entryAccount =
    accounts.find(
      (account) => account.id === entry.accountId && account.group === "cash",
    ) ?? null;
  const entryAmount = Number(entry.amount);
  const hasPreviewAmount = Number.isFinite(entryAmount) && entryAmount > 0;
  const entryBalanceAfter =
    entryAccount && hasPreviewAmount
      ? entryAccount.balance +
        (entry.type === "income" ? entryAmount : -entryAmount)
      : null;
  ...
  function submitEntry(event: React.FormEvent) {
    ...
    const balanceAfter =
      ...
      (entry.type === "income" ? amount : -amount);
  ...
  function submitAccount(event: React.FormEvent) {
    ...
    setAccounts((current) => [
      ...current,
      {
        ...
        balance,
        note:
          accountForm.note.trim() ||
          accountGroups.find((group) => group.id === addingGroup)?.title ||
          "账户",
  ...
  function submitAccountActivity(event: React.FormEvent) {
    ...
    if (activityForm.purposeType === "银行卡转账") {
      const targetAccount = accounts.find(
        (account) => account.id === activityForm.transferAccountId,
      );
      if (!targetAccount) {
    ...
    const balanceAfter =
      ...
      (activityForm.type === "income" ? amount : -amount);
  ...
  function confirmDeleteAccount() {
    ...
    setTransactions((current) =>
      current.filter(
        (transaction) => transaction.accountId !== deleteAccount.id,
      ),
    );
    setAccountActivities((current) =>
      current.filter((activity) => activity.accountId !== deleteAccount.id),
    );
    setEntry((current) =>
      current.accountId === deleteAccount.id
        ? {
            ...current,
            accountId:
              remainingAccounts.find((account) => account.group === "cash")
                ?.id ?? "",
          }
        : current,
    );
    ...
    setTransactions((current) =>
      current.map((transaction) =>
        transaction.accountId === selectedAccount.id
          ? { ...transaction, accountName: nextName }
          : transaction,
      ),
    );
    setRenameValue("");
  ...
  return (
    <>
      ...
      <header
        ...
        aria-label="全局导航"
        data-shell-surface="calm"
      >
        <button
          className="brand-mark"
          type="button"
          onClick={() => setActiveView("finance")}
          aria-label="Dice Life 首页"
        >
          <strong>Dice Life</strong>
          <span className="brand-die" aria-hidden="true">
            <i />
            <i />
            <i />
            <i />
        ...
        <nav className="top-nav" aria-label="主导航">
          <button
            ...
            aria-current={activeView === "quests" ? "page" : undefined}
            type="button"
        ...
        <div className="header-tools">
          <button className="round-tool" type="button" aria-label="通知">
            <Bell size={20} />
          </button>
          <button className="round-tool" type="button" aria-label="设置">
            <Settings size={20} />
          </button>
          <div className="avatar-badge" aria-label="角色头像">
            42
          </div>
        </div>
      </header>
      ...
      <aside
        className={`app-sidebar${activeView === "body" ? " health-sidebar" : ""}`}
        aria-label="角色与模块导航"
        data-shell-surface="calm"
      >
        <button
        ...
        <nav className="side-nav" aria-label="系统模块">
          <button
            ...
            onClick={() => setActiveView("finance")}
          >
            <CircleDollarSign size={19} /> 财富状况
          </button>
          <button
            className={activeView === "ability" ? "active" : ""}
            aria-current={activeView === "ability" ? "page" : undefined}
            type="button"
          ...
          <button
            className={activeView === "body" ? "active" : ""}
            aria-current={activeView === "body" ? "page" : undefined}
            type="button"
            onClick={() => setActiveView("body")}
          >
            <HeartPulse size={19} /> 健康状况
          </button>
          <button
            className={activeView === "emotion" ? "active" : ""}
            aria-current={activeView === "emotion" ? "page" : undefined}
            type="button"
        ...
        <section className="hero-stats" aria-label="英雄属性">
          ...
          <div className="xp-row">
            <span>健康度</span>
            <div className="xp-bar">
              <i className="emerald-fill" style={{ width: "88%" }} />
            </div>
          </div>
          <div className="xp-row">
            <span>情绪</span>
            <div className="xp-bar">
              <i className="ruby-fill" style={{ width: "45%" }} />
            </div>
          </div>
        </section>
      ...
      <main
        className={`app-shell${activeView === "emotion" ? " emotion-shell" : ""}${activeView === "body" ? " health-shell" : ""}`}
        id="main-content"
        tabIndex={-1}
      >
        {activeView === "finance" ? (
          <>
            <section className="hero-panel wealth-hero">
              <div className="hero-copy">
                <div className="eyebrow">
                  <CircleDollarSign size={18} /> 经济系统 · {releaseVersion} ·{" "}
                  {releaseDate}
                </div>
                <h1>净资产</h1>
                ...
                <p>
                  净资产 = 资金账户 + 理财账户 + 应收账款 + 不动产估值 -
                  负债。账户下拉页可以新增账户，点击账户进入子页面记录收入或支出。
                </p>
              </div>
              <div
                className="score-card"
                aria-label={`经济评分 ${economyScore} 分`}
              >
                <span>经济评分</span>
            ...
            <section className="metric-grid" aria-label="经济关键指标">
              ...
              <article className="metric-card">
                ...
                <span>本月支出</span>
                <strong aria-label="本月支出金额">
                  {formatCurrency(overview.monthlyExpense)}
                </strong>
            ...
            <section className="account-grid" aria-label="账户总览">
              {accountGroups.map((group) => {
                ...
                return (
                  <article
                    className={`account-card ${isOpen ? "open" : ""}`}
                    key={group.id}
                  >
                    <button
            ...
            <section className="cashflow-log panel" aria-label="本月收入支出">
              <button
                className="cashflow-log-header"
                type="button"
                onClick={() => setIsCashflowPageOpen(true)}
              >
                <span>
                ...
                <span className="cashflow-log-summary">
                  ...
                  <small>
                    收入 {formatCurrency(overview.monthlyIncome)} / 支出{" "}
                    {formatCurrency(overview.monthlyExpense)}
                  </small>
                </span>
                <BarChart3 size={22} />
        ...
        {isCashflowPageOpen ? (
          <div
            className="sheet-backdrop cashflow-page-backdrop"
            role="presentation"
            onClick={() => setIsCashflowPageOpen(false)}
          >
            <section
            ...
            >
              ...
              <div className="cashflow-filter-bar">
                <label className="ledger-date-select">
                  <span>{period}</span>
                  <select
                    value={activeLedgerDate}
                    onChange={(event) =>
                      setSelectedLedgerDate(event.target.value)
                    }
                  >
                ...
                <span>资金账户</span>
                <span>按金额</span>
                <button className="text-filter-button" type="button">
                  筛选
                </button>
              </div>
              ...
              <div className="cashflow-page-grid">
                ...
                <section className="ledger-day-panel" aria-label="每日收支记录">
                  ...
                  <div
                    className="cashflow-record-list"
                    role="region"
                    aria-label="本月收入支出记录明细"
                  >
                    {activeDateTransactions.length > 0 ? (
                      activeDateTransactions.map((transaction) => (
                        <div
                          className="cashflow-record-row"
                          key={transaction.id}
                        >
                          <span>
                            <strong>
              ...
              {entryAccount ? (
                <div className="balance-preview" aria-label="资金账户余额预览">
                  ...
                  {entryBalanceAfter !== null ? (
                    <small
                      className={
                        entryBalanceAfter >= entryAccount.balance
                          ? "income"
                          : "expense"
                      }
                    >
                      {entry.type === "income" ? "入账后余额" : "扣除后余额"}：
                      {formatCurrency(entryBalanceAfter)}
                    </small>
                  ) : (
            ...
            >
              <div className="sheet-header">
                <div>
                  <p>
                    {
                      accountGroups.find((group) => group.id === addingGroup)
                        ?.title
                    }
                  </p>
                  <h2>
        ...
        {selectedCalendarMonth ? (
          ...
          >
            <div
              className="entry-sheet account-detail-sheet"
              role="dialog"
              aria-label="月份收支来源"
              onClick={(event) => event.stopPropagation()}
            >
        ...
        {selectedAccount ? (
          ...
          >
            <form
              className="entry-sheet account-detail-sheet"
              onSubmit={submitAccountActivity}
              aria-label="账户详情表单"
              onClick={(event) => event.stopPropagation()}
            >
              <div className="sheet-header">
                  ...
                  {renameValue ? (
                    ...
                    <div className="account-title-row">
                      ...
                      <button
                        className="icon-button"
                        type="button"
                        aria-label={`重命名${selectedAccount.name}`}
                        onClick={startRenameAccount}
                      >
                        <Pencil size={18} />
                      </button>
                    </div>
              ...
              <div className="account-balance-panel">
                ...
                <small>
                  {[
                    ...
                    selectedAccount.cost
                      ? `成本 ${formatCurrency(selectedAccount.cost)}`
                      : "",
                    selectedAccount.dueDate
                      ? `预计到账 ${selectedAccount.dueDate}`
                      : "",
                  ]
                    .filter(Boolean)
                    .join(" · ")}
              ...
              <fieldset className="segmented">
                ...
                {(["expense", "income"] as TransactionType[]).map((type) => (
                  <label
                    key={type}
                    className={activityForm.type === type ? "selected" : ""}
                  >
                    <input
                      type="radio"
                    ...
                    />
                    {type === "expense"
                      ? operationLabels.expense
                      : operationLabels.income}
                  </label>
                ))}
              </fieldset>
              <label className="field">
                  ...
                  selectedAccountMonths.map((month) => {
                    ...
                    return (
                      <section className="monthly-bill-card" key={month.month}>
                        <button
                          className="monthly-bill-header"
                          type="button"
                          onClick={() =>
                            setOpenAccountMonth(isOpen ? null : month.month)
                          }
                        >
                          <span>
                          ...
                          <span className="monthly-bill-total">
                            <b className="expense">
                              流出: {formatCurrency(month.expense)}
                            </b>
                            <b className="income">
                              流入: {formatCurrency(month.income)}
                            </b>
                          </span>
                          <ChevronDown
                        ...
                        {isOpen ? (
                          <div className="monthly-bill-detail">
                            {groupActivitiesByDate(month.items).map(
                              (dateGroup) => (
                                <div
                                  className="bill-date-group"
                                  key={dateGroup.date}
                                >
                                  ...
                                  {dateGroup.items.map((activity) => (
                                    ...
                                    >
                                      <span>
                                        ...
                                        <small>
                                          {activity.relatedAccountName
                                            ? `关联账户: ${activity.relatedAccountName} · `
                                            : ""}
                                          余额:{" "}
                                          {formatCurrency(
                                      ...
                                      <b
                                        className={
                                          isActivityInflow(activity.type)
                                            ? "income"
                                            : "expense"
                                        }
                                      >
