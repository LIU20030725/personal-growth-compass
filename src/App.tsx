    ...
    <>
      ...
      <header className="app-header" aria-label="全局导航" data-shell-surface="calm">
        ...
        <nav className="top-nav" aria-label="主导航">
          <button className={activeView === 'quests' ? 'active' : ''} aria-current={activeView === 'quests' ? 'page' : undefined} type="button" onClick={() => openView('quests')}>任务</button>
          <button className={activeView === 'achievements' ? 'active' : ''} aria-current={activeView === 'achievements' ? 'page' : undefined} type="button" onClick={() => openView('achievements')}>成就</button>
          <button className={activeView === 'journal' ? 'active' : ''} aria-current={activeView === 'journal' ? 'page' : undefined} type="button" onClick={() => openView('journal')}>冒险日志</button>
        </nav>
      ...
      <aside className={`app-sidebar${activeView === 'body' ? ' health-sidebar' : ''}`} aria-label="角色与模块导航" data-shell-surface="calm">
        ...
        <nav className="side-nav" aria-label="系统模块">
          ...
          <button className={activeView === 'emotion' ? 'active' : ''} aria-current={activeView === 'emotion' ? 'page' : undefined} type="button" onClick={() => openView('emotion')}><Smile size={19} /> 情绪状态</button>
        </nav>
