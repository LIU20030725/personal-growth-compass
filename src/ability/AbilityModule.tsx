...
import { AbilityTreeStage, removePhaseCanvasPreference, resetCanvasLayoutPreferences } from './components/AbilityTreeStage';
...
export function AbilityModule({ abilityStorage, initialTreeId = null, onTreeChange }: Props) {
  ...
  return <section ref={moduleRef} className="ability-module" aria-label="能力属性模块">
    ...
    {form === 'edit-phase' && currentTree && editingPhaseId ? <PhaseFormDialog initial={phases.find((phase) => phase.id === editingPhaseId)} onClose={() => { setEditingPhaseId(null); setForm(null); }} onSave={(value) => { ability.updatePhase(editingPhaseId, value); setEditingPhaseId(null); setForm(null); }} onDelete={() => { const phaseId = editingPhaseId; ability.removePhase(phaseId); if (currentCanvasHistory?.present.phasePositions[phaseId]) commitCanvasPreferences(removePhaseCanvasPreference(currentCanvasHistory.present, phaseId)); setEditingPhaseId(null); setForm(null); }} deleteDisabledReason={nodes.some((node) => node.phaseId === editingPhaseId) ? '请先移动或归档阶段内的技能节点' : undefined} /> : null}
