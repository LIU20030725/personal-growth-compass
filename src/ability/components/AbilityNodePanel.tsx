...
export function AbilityNodePanel(props: Props) {
  ...
  const needsNote = props.criteria.some((criterion) => !criterion.satisfied);
  ...
  return <aside className="ability-node-panel" aria-label="技能节点详情">
    ...
    <section className="ability-detail-section"><div className="ability-detail-heading"><h3>掌握标准</h3><span>{props.criteria.filter((item) => item.satisfied).length}/{props.criteria.length}</span></div>
      ...
      {node.progress !== 'mastered' && props.displayState !== 'locked' ? <div className="ability-mastery-confirm">
        {needsNote ? <label><span>提前掌握说明</span><textarea aria-label="提前掌握说明" value={masteryNote} onChange={(event) => setMasteryNote(event.target.value)} placeholder="标准未全部满足时，请说明你的判断依据" /></label> : null}
        <button className="ability-primary ability-wide" type="button" onClick={() => props.onConfirmMastery(masteryNote)}>确认已掌握</button>
      </div> : null}
